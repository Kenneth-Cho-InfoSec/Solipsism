...
public class DownloadHandler {
    ...
    public void onDownloadStart(@NonNull Activity context,
        ...
        logger.log(TAG, "DOWNLOAD: request host=" + originalUri.getHost() + " mime=" + mimeType);
    ...
    private void onDownloadStartNoStream(@NonNull final Activity context,
        ...
        try {
            ...
            // exception here
            logger.log(TAG, "Exception while trying to parse download URL", e);
        ...
        if (!URLUtil.isNetworkUrl(addressString)) {
        ...
        String newMimeType = MimeTypeMap.getSingleton().getMimeTypeFromExtension(Utils.guessFileExtension(sanitizedFilename));
        logger.log(TAG, "New mimetype: " + newMimeType);
        request.setMimeType(newMimeType);
        request.setDestinationUri(Uri.parse(Constants.FILE + location + sanitizedFilename));
        ...
        if (cookies != null && !cookies.isEmpty() && "https".equalsIgnoreCase(uri.getScheme())) {
        ...
        if (mimetype == null) {
            ...
            try {
                ...
                ActivityExtensions.snackbar(context, context.getString(R.string.download_pending) + ' ' + sanitizedFilename);
        }
    }
